...
class Validator {
  ...
  validatePassword(field, value, minLength = 8) {
  ...
  minLength(field, value, length) {
    if (value !== undefined && value.length < parseInt(length)) {
      this.addError(field, `${field} must be at least ${length} characters long.`);
    }
  }

  maxLength(field, value, length) {
    if (value !== undefined && value.length > parseInt(length)) {
      this.addError(field, `${field} must be no more than ${length} characters long.`);
    }
  }
...
module.exports = { Validator, autoTrim, normalizeEmail, validatePassword: Validator.prototype.validatePassword };